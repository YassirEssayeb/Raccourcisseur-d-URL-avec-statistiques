    ...
    <div className="min-h-dvh bg-[#f8fafc] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-[400px]">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-[#0f172a] mb-1">Créer un compte</h1>
          <p className="text-sm text-[#64748b]">Raccourcissez vos liens en un clic</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-[0_4px_24px_rgba(0,0,0,0.04)] space-y-4">
          {pendingUrl && (
            <div className="text-xs text-[#475569] bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-3.5 py-2.5 truncate">
              URL à raccourcir : <span className="text-[#0f172a] font-medium">{pendingUrl}</span>
            </div>
          )}
          ...
          {pendingUrl && (
            <p className="text-xs text-[#94a3b8] text-center">
              Votre URL sera raccourcie automatiquement après inscription.
            </p>
          )}
        </form>
        ...
        <p className="text-center text-sm text-[#94a3b8] mt-6">
          Déjà un compte ?{' '}
          <Link to="/login" className="text-[#14b8a6] font-semibold hover:text-[#0d9488] transition-colors">
            Se connecter
          </Link>
        </p>
      </div>
